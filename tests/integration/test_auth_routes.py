...
def test_login_unknown_user_returns_200(client):
    rv = login(client, username="nobody_here", password="Whatever1!")
    assert rv.status_code == 200
...
def test_login_respects_safe_next_parameter(client, credentials_user):
    rv = login_as(
        client,
        credentials_user,
        login_path="/auth/login?next=/assets/",
    )
    assert rv.status_code == 302
